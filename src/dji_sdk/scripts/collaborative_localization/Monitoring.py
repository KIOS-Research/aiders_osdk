...
Christos Georgiades - chr.georgiades@gmail.com - 2023
"""
...
script_dir = os.path.realpath(__file__).replace('/Monitoring.py', '')
print('=*= Monitoring.py =*=')
...
def main_TM():
    ...
    global monitoring_state_enabled
    ...
    time.sleep(1.0)
    key = None
    while monitoring_state_enabled and not rospy.is_shutdown():
        ...
        while(monitoring_state_enabled and vs.more() and not rospy.is_shutdown()):
            ...
            if (image is not None):
                ...
                if save_results:
                    if frame_num == 0:
                        ...
                        vid_out = cv2.VideoWriter(flightlog_path + 'matrice300_' + boardId + '-' + initTime + '-monitoring_stream.avi', cv2.VideoWriter_fourcc('A','V','C','1'), vid_fps, (Width, Height))
    ...
    print("\nDuration:",round(time.time()-start_time),"seconds")
...
def set_monitoring_state(state_enable):
    global monitoring_state_enabled

    if state_enable:
        if not monitoring_state_enabled:
            monitoring_state_enabled = True

            monitoring_thread = Thread(target=main_TM)
            monitoring_thread.start()
    else:
        monitoring_state_enabled = False


def init():
    global crps_data
    global detector_resolution_pub
...
print('Initializing CRPS Monitoring...')
