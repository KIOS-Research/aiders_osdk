...
def createTable(packetType, packet):        
    if dbUtils.DEBUG:
...
def create_table(table_name, topic_type, column_names_tuple):
    #topic_type = message_instance._type
    #message_serialized = message_converter.convert_ros_message_to_dictionary(message_instance)


    
    print('Creating Table: ' + table_name + '\n\tType: ' + topic_type + '\n\tSize:' + str(len(column_names_tuple)) + '\n\tColumns:' + str(column_names_tuple) + '\n')

    query = "CREATE TABLE '" + table_name + "' (" + ",".join(column_names_tuple) + ")"
    if dbUtils.DEBUG:
        print(query)
...
def save_serialized_data(table_name, serialized_packet):
    #sql_values = [tuple(serialized_packet.values())]

    print('serialized_packet:', serialized_packet)
    #print('sql_values:', sql_values)
    
    for key in serialized_packet.keys():
        data = serialized_packet[key]
        if isinstance(data, dict):
            data = str(data)
            serialized_packet[key] = data
    ...
    for i in range(len(serialized_packet) - 1):
        query += '?, '
    query += '?)'

    dbUtils.executeQueryWrite(query, [tuple(serialized_packet.values())])
