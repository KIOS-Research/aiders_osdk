...
requestTableLock = {}  
requestTableMutex = threading.Lock()    
...
total_stored_packets_mutex = threading.Lock()
def increment_stored_packets_CB(packet_count):
    global total_stored_packets
    new_stored_packets = packet_count.data

    with total_stored_packets_mutex:
        total_stored_packets = total_stored_packets + new_stored_packets
        packet_count_pub.publish(total_stored_packets)


def listener(dji_name = "matrice300"):
    global boardId
    global flightlog_path
    global packet_count_pub 
    ...
    rospy.Subscriber(dronename+'/Database/increment_stored_packets', UInt32, increment_stored_packets_CB)

    packet_count_pub = rospy.Publisher(dronename+'/Database/Get_total_stored_packets', UInt32, queue_size=5)
...
# Topics to be saved
def subscribeTopics():
    print('Setting up topic workers...')
    dronename = dbUtils.dbInstanceName

    # Init
    database_worker.DatabaseWorker(dronename+'/FlightLogDirectory', String, expected_frequency=1, timestamp_entries=True)
    database_worker.DatabaseWorker(dronename+'/Telemetry/HomePosition', Telemetry, expected_frequency=1, timestamp_entries=True)
    
    # Link Stats
    database_worker.DatabaseWorker('/master_discovery/linkstats', LinkStatesStamped, timestamp_entries=True)


    database_worker.DatabaseWorker(dronename+'/ControlAuthority/Get_State_Enable', ControlAuthorityRequest, expected_frequency=1, timestamp_entries=True)
    
    # Telemetry
    database_worker.DatabaseWorker(dronename+'/telemetry2', Telemetry, expected_frequency=50)
    database_worker.DatabaseWorker(dronename+'/Telemetry', Telemetry, expected_frequency=50)

    # Metrics
    database_worker.DatabaseWorker(dronename+'/DroneMovement', DroneMovement, expected_frequency=5)
    database_worker.DatabaseWorker(dronename+'/TerminalHardware', TerminalHardware, expected_frequency=5, timestamp_entries=True)
    database_worker.DatabaseWorker(dronename+'/DroneHardware', DroneHardware, expected_frequency=5)

    # CRPS
    database_worker.DatabaseWorker(dronename+'/PSP/Source', String, expected_frequency=1)
    database_worker.DatabaseWorker(dronename+'/Log/List', ErrorLogList, expected_frequency=1)
    database_worker.DatabaseWorker('/crps/Telemetry', Telemetry, expected_frequency=1)
    database_worker.DatabaseWorker(dronename+'/crps/Telemetry', Telemetry, expected_frequency=1)
    
    database_worker.DatabaseWorker('/crps/Telemetry/Set_State_Enable', Bool, expected_frequency=1, timestamp_entries=True)
    database_worker.DatabaseWorker(dronename+'/crps/Request', Bool, expected_frequency=1, timestamp_entries=True)
    database_worker.DatabaseWorker(dronename+'/crps/State', Bool, expected_frequency=1, timestamp_entries=True)

    database_worker.DatabaseWorker(dronename+'/crps/initiate_collaboration_request/Get_State_Enable', Bool, expected_frequency=1, timestamp_entries=True)
    database_worker.DatabaseWorker(dronename+'/crps/initiate_collaboration_request/Set_State_Enable', CrpsRequest, expected_frequency=1, timestamp_entries=True)

    database_worker.DatabaseWorker(dronename+'/crps/handle_collaboration_request/Set_State_Enable', CrpsRequest, expected_frequency=1, timestamp_entries=True)
    database_worker.DatabaseWorker(dronename+'/crps/handle_collaboration_request/Get_State_Enable', Bool, expected_frequency=1, timestamp_entries=True)

    database_worker.DatabaseWorker(dronename+'/crps/initiate_collaboration_request/acknowledgments/Ack', CrpsRequest, expected_frequency=1, timestamp_entries=True)
    database_worker.DatabaseWorker(dronename+'/crps/initiate_collaboration_request/acknowledgments/Ack_List', CrpsRequestAckList, expected_frequency=1, timestamp_entries=True)

    # Joy
    database_worker.DatabaseWorker(dronename+'/rc', Joy)
    database_worker.DatabaseWorker(dronename+'/joy', Joy)
    database_worker.DatabaseWorker('/joy', Joy)
    database_worker.DatabaseWorker(dronename + '/joy/smoothed_joy', Quaternion)
    database_worker.DatabaseWorker(dronename + '/joy/Get_Smooth_Mode', Int8, expected_frequency=1, timestamp_entries=True)
    database_worker.DatabaseWorker(dronename + '/joy/Get_Lerp_Rate', Float64, expected_frequency=1, timestamp_entries=True)

    # Database Metrics
    database_worker.DatabaseWorker(dronename+'/Database/Get_total_stored_packets', UInt32, expected_frequency=1, timestamp_entries=True, publish_stats=False)
