...
# Performs thread safe database write
def executeQueryWriteMany(table_name, values):
    global dbInstance, dbCursor, dbMutex

    data_size = len(values[0])
    #print('executeQueryWriteMany - data_size:', data_size)
...
def Init(dronename):
    ...
    try:
        flightlog_path_pckt = rospy.wait_for_message(dronename + '/FlightLogDirectory', String, timeout=1)
        ...
        flightlog_path = log_path + dronename.replace('/', '') + '-' + datetime.datetime.now().strftime("%Y%m%d-%H:%M:%S.%s")+ '/'
