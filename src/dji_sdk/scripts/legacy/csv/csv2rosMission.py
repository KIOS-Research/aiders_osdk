...
def finishMission():
    ...
    #print('Building 3dMesh...')
    #imgDirectory = '/home/jetson/Documents/mesh_construction/input_images/'
...
def listener(dji_name = "/matrice300"):
    ...
    nodename = dronename.replace('/', '') + '_mission'
    print(nodename)
    
    rospy.init_node(nodename, anonymous=False)    
    #fc.listener(dji_name = dronename)
    
    missionCommandSub = rospy.Subscriber(dji_name+'/Mission', MissionDji, MissionCommandCB);
    responsePub = rospy.Publisher(dji_name+'/ScanAreaResponse', MissionWaypointTask, queue_size=2)
        
    #missionUploadSub = rospy.Subscriber("matrice300/mission_waypoint_report", String, startMission);
    
    
    droneStateSub = rospy.Subscriber(dji_name+'/DroneState', String, DroneStateCB)
