...
class VideoStream:
    def __init__(self,  transform=None, queue_size=512,detectNet=None,resize=True,size=(512,512),skip=5, dronename='matrice300', use_aruco=False):
        ...
        if self.use_aruco:
            ...
            self.detectNet = cv2.aruco.ArucoDetector(self.aruco_dict, self.aruco_params)
            print('VideoStream - Aruco initialization finished')
        else:
            self.detectNet = cv2.QRCodeDetector()
            print('VideoStream - QRCodeDetector initialization finished')
...
# python
# Copy code
# aruco_dict = aruco.Dictionary_get(aruco.DICT_4X4_50)  # You can choose different dictionary sizes
            ...
            if self.streamframes != self.callbackframes:
                ...
                if  self.frameGL is not None:
                    ...
                    try:
                        ...
                        #print('queue_thread - detecting...')
                        if self.use_aruco:
                            corners, ids, rejected = self.detectNet.detectMarkers(frame)
                            ...
                            self.detsGL = (retval, ids, corners, rejected)
                        else:
                            retval, decoded_info, points, straight_qrcode = self.detectNet.detectAndDecodeMulti(frame)
                            self.detsGL = (retval, decoded_info, points, straight_qrcode)

                        #if retval:
                        #    print('queue_thread - Got detections')
